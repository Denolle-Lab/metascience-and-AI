...
- Later again: the entry-state table gained a geoscience column, one or two papers per state (Obara 2002; Dragert et al. 2001; Raff and Mason 1961; Alvarez et al. 1980; Heaton and Kanamori 1984; Brune et al. 1969; Bakun and Lindh 1985; Bakun et al. 2005; Massonnet et al. 1993; Shapiro et al. 2005; Okada et al. 2004; Field et al. 2014; Johnson et al. 2021). Ten records added as meeting 1 optional readings; three were already registered under meetings 5 and 9. Titles, venues, volumes, pages, and DOIs were checked against publisher and index pages by web search; Crossref was not reachable from the session, so `tools/check_links.py` has not been run on them. The Pandoc citation pass could not run either (no Pandoc in the session); the remaining validator checks pass.
